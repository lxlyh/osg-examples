...
OSGWidget::OSGWidget(QWidget* parent) : QOpenGLWidget (parent)
{
    ...
    //读取带动画的节点
    auto animationNode = osgDB::readNodeFile("../OpenSceneGraph-Data/animations-2.fbx");

    //获得节点的动画列表
    osgAnimation::BasicAnimationManager* anim = dynamic_cast<osgAnimation::BasicAnimationManager*>(animationNode->getUpdateCallback());
    const osgAnimation::AnimationList& list = anim->getAnimationList();

    //从动画列表中选择一个动画，播放
    anim->playAnimation(list[0].get());

    world->addChild(animationNode);
